...
- `GeoOrbitRegistry` — live Proof-of-Space-Time telemetry anchor for the GeoOrbit RTK mesh: stations register, PoS-chained heartbeats (speed-bounded, no teleport), on-chain ORBIT reward accounting: `0x8829e0e397ffa872D3E5B5CE5d32b52E98DC4e63`
- Evidence Registry — proof-gated events (oracle anchors, escrow releases, metered/prepaid usage, GeoOrbit telemetry anchors) recovered via `eth_getLogs` from the deployed contracts
Reproduce with `npm run deploy:policy` (purpose + metering), `npm run deploy:flagship` (escrow + prepaid-meter v2) and `npx hardhat run scripts/deploy_geoorbit.js --network creditcoinTestnet`.
