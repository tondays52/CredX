...
> **Positioning:** CredX is the attestation backbone and policy layer, not one more credit passport. Where the field ships single one-feature apps that "prove one event on one chain", CredX ships **five integrated tracks on one verified spine** — 22 deployed contracts, a full automated test suite, a **RiskGuard verify-then-execute policy gate**, a **live Covenant Ops / collateral-liveness feed** driven directly by the deployed oracle, and an **enterprise policy layer** (purpose-bound RWA funding, prepaid-metered usage, verified escrow, and an on-chain Evidence Registry). CredX is the platform; the pack is single-function applications.
...
Shipping a lender-grade product means the proof alone is not enough — the *decision* must be enforced by contract, not by whoever called the function. CredX ships six coordinate layers on top of the oracle:
...
4. **Metered Accountable Usage + Prepaid Credits** (`contracts/core/UsageMeteringRegistry.sol`, live on testnet as prepaid v2 at `0xF8a9645ac3D234cf72B0C4C170cFB289FE2Ae4F9`)
   Meters are registered per wallet + action key (GPU lease, flash-loan cycles, compute ops, arena ticks) with a unit price and a window cap. The meter advances **only** on a verified attested receipt (public proof path) or a registered KYC agent (telemetry path); window caps **fail closed** (`ExceedsWindowCap` reverts the increment); accrued debt is on-chain and payable in cUSD. New in v2: a **prepaid credit balance** is consumed **first, fail-closed** — while prepaid exists a debit that exceeds it reverts (`InsufficientPrepaid`) instead of silently drifting into debt; top-ups and withdrawals are settlement-token transactions. `frontend/src/components/terminal/MeterTrackingView.tsx` reads meters + prepaid live and mirrors the semantics in a labeled simulated feed.
5. **Verified Escrow** (`contracts/core/VerifiedEscrow.sol`, live on testnet at `0x07aBcbb7b2F9f4400c93d092F343e186ee526137`)
   A depositor locks cUSD against an order. The escrow releases **only** when a cross-chain receipt verifies against the Attestcoin verifier — the exact seller, order reference, amount and deadline are bound at creation, funds are unreleasable without a valid proof until the deadline, and after the deadline anyone can trigger the refund (funds can never be locked forever). A receipt spent on one release is **replay-guarded**: `keccak(sourceChainId, txHash)` is consumed, so the same attested transaction can never pay twice across escrows. `frontend/src/components/terminal/VerifiedEscrowView.tsx` lists live escrow jobs (locked/released/refunded) and signs real release/refund transactions with a connected wallet.
6. **Evidence Registry** (`frontend/src/components/terminal/EvidenceRegistryView.tsx`)
   Every proof-gated event is recoverable on-chain: BlockProver **attestation anchors** (`ProofAnchored`), **escrow releases** (`EscrowReleased`) and **metered/prepaid usage** (`UsageRecorded`, `PrepaidConsumed`) are queried from the deployed contracts via `eth_getLogs` — the same registry surface a dispute auditor or judge queries. Nothing can be quietly reversed because the registry never forgets who proved what, and when.
...
* **Automated Unit & Integration Test Suite**: **126 passing tests (100% pass rate, ~8s)** covering all 5 hackathon tracks:
  ...
  - Enterprise policy layer (19 tests): purpose-bound recipient enforcement, attested-tranche gating, covenant freeze/settle, meter caps (fail-closed), window rollover, KYC-agent telemetry, cUSD debt settlement, prepaid top-up/withdraw/consume (prepaid-first fail-closed)
  - VerifiedEscrow (8 tests): condition-locked funding, proof-gated release, replay-receipt rejection, seller/order/deadline binding, deadline refund
...
| **UsageMeteringRegistry (prepaid v2: metered + prepaid-first)** | `0xF8a9645ac3D234cf72B0C4C170cFB289FE2Ae4F9` | Creditcoin Testnet |
| **VerifiedEscrow (condition-locked, proof-gated settlement)** | `0x07aBcbb7b2F9f4400c93d092F343e186ee526137` | Creditcoin Testnet |
