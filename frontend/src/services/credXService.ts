...
// ─── Real on-chain perpetuals (ReputationPerpetual on the live AMM mark) ─────
...
export const PERP_ABI = [
  ...
  'function closePosition(uint64 id) returns (uint256 payoutUsd)',
  'function addMargin(uint64 id, uint256 amount)',
  'function removeMargin(uint64 id, uint256 amount)',
  ...
  'event PositionClosed(address indexed owner, uint64 id, uint8 side, uint256 realizedUsd, uint256 mark, uint256 fee)',
...
export interface PerpProjection {
  notional: number;
  fee: number;
  collateralAfterFee: number;
  liqPriceApprox: number;
}

export const PERP_EVENTS = {
  PositionOpened: ethers.id('PositionOpened(address,uint64,uint8,uint256,uint256,uint256,uint256)'),
  PositionClosed: ethers.id('PositionClosed(address,uint64,uint8,uint256,uint256,uint256)'),
  PositionLiquidated: ethers.id('PositionLiquidated(address,uint64,address,uint256,uint256)'),
  MarginChanged: ethers.id('MarginChanged(uint64,uint256,bool)'),
  FundingApplied: ethers.id('FundingApplied(uint256,uint256,uint256)'),
  InsuranceTopUp: ethers.id('InsuranceTopUp(address,uint256)'),
} as const;

const PERP_SCALE = 10n ** 18n;

/** Constants mirrored from the deployed ReputationPerpetual. */
const PERP_LONG_PAYS = 1 as const;

function perpPriceToNumber(raw: bigint): number {
  return Number(raw) / 1e18;
}

export async function fetchPerpMarket(user: string): Promise<PerpMarketState | null> {
  try {
    const c = readContract(CONTRACTS.reputationPerpetual, PERP_ABI);
    const hub = hubRead();
    const [market, finf, pool, fee, profile] = await Promise.all([
      c.marketInfo(),
      c.fundingInfo(),
      c.insurancePool(),
      c.feeBpsFor(user),
      hub.getBorrowerProfile(user).catch(() => null),
    ]);
    const markRaw = market.mark;
    return {
      mark: perpPriceToNumber(markRaw),
      markRaw,
      creditScore: profile ? Number(profile.creditScore) : 0,
      feeBps: Number(fee),
      maxLeverage: Number(market.maxLev) / 10000,
      maintenanceBps: Number(market.maint),
      liquidationBonusBps: Number(market.liqBonus),
      maxPositionNotional: parseFloat(ethers.formatUnits(market.maxPos, 18)),
      maxTotalNotional: parseFloat(ethers.formatUnits(market.maxTotal, 18)),
      longNotional: parseFloat(ethers.formatUnits(market.longNotional, 18)),
      shortNotional: parseFloat(ethers.formatUnits(market.shortNotional, 18)),
      openCount: Number(market.openCount),
      reserveQuote: parseFloat(ethers.formatUnits(market.reserveQuote, 18)),
      reserveBase: parseFloat(ethers.formatUnits(market.reserveBase, 18)),
      insurancePool: parseFloat(ethers.formatUnits(pool, 18)),
      fundingRateBps: Number(finf.rateBps),
      fundingEpochBlocks: Number(finf.epochBlocks),
      lastFundingBlock: Number(finf.lastBlock),
      nextFundingBlock: Number(finf.nextFundingBlock),
      fundingAcc: perpPriceToNumber(finf.acc),
      currentBlock: Number(await readProvider().getBlockNumber()),
    };
  } catch {
    return null;
  }
...
/** Same realized math the contract uses: mark delta + funding accumulator delta. */
export function perpRealized(side: 'LONG' | 'SHORT', entryMark: number, markNow: number, notional: number, fundingAtOpen: number, fundingAccNow: number): { pnl: number; fundingDelta: number } {
  const entry = entryMark > 0 ? entryMark : markNow;
  const dirPnl = side === 'LONG' ? markNow - entry : entry - markNow;
  const pnl = markNow === entry ? 0 : (dirPnl / entry) * notional;
  const accrued = Math.min(fundingAccNow - fundingAtOpen, 1);
  const funding = accrued * notional;
  const fundingDelta = side === 'LONG' ? -funding : funding;
  return { pnl, fundingDelta };
}

export async function fetchPerpPositions(user: string): Promise<PerpPosition[]> {
  try {
    const c = readContract(CONTRACTS.reputationPerpetual, PERP_ABI);
    const [ids, markRaw, fundingNow] = await Promise.all([c.positionsOf(user), c.markPrice(), c.fundingInfo()]);
    const mark = perpPriceToNumber(markRaw);
    const fundingAcc = perpPriceToNumber(fundingNow.acc);
    const maintenanceBps = Number((await c.marketInfo()).maint);
    const out: PerpPosition[] = [];
    for (const id of ids) {
      const p = await c.getPosition(id);
      if (p.closed) continue;
      const side: 'LONG' | 'SHORT' = Number(p.side) === 0 ? 'LONG' : 'SHORT';
      const collateral = parseFloat(ethers.formatUnits(p.collateral, 18));
      const notional = parseFloat(ethers.formatUnits(p.notional, 18));
      const entryMark = perpPriceToNumber(p.entryMark);
      const { pnl, fundingDelta } = perpRealized(side, entryMark, mark, notional, perpPriceToNumber(p.fundingAtOpen), fundingAcc);
      const equity = collateral + pnl + fundingDelta;
      const maintenanceReq = (notional * maintenanceBps) / 10000;
      out.push({
        id: id.toString(),
        ...
        notional,
        entryMark,
        fundingAtOpen: perpPriceToNumber(p.fundingAtOpen),
        openedAtBlock: Number(p.openedAtBlock),
        markNow: mark,
        unrealizedPnl: pnl,
        fundingDelta,
        equity,
        maintenanceReq,
        marginRatio: notional > 0 ? equity / notional : 0,
...
export function projectPerpPosition(side: 'LONG' | 'SHORT', collateral: number, leverage: number, mark: number, feeBps: number): PerpProjection {
  const notional = (collateral * leverage) / 1;
  const fee = (notional * feeBps) / 10000;
  const collateralAfterFee = Math.max(0, collateral - fee);
  const levEff = notional > 0 ? notional / Math.max(1, collateralAfterFee) : 0;
  const liqPriceApprox = side === 'LONG' ? mark * (1 - 1 / levEff) : mark * (1 + 1 / levEff);
  return { notional, fee, collateralAfterFee, liqPriceApprox };
}

export async function executeOpenPerp(side: 'LONG' | 'SHORT', collateral: number, leverage: number, signer?: ethers.Signer): Promise<{ txHash: string; block: number; id: string | null; notional: number }> {
  const s = signer ?? demoWalletSigner(DEMO_WALLET_VAULT.find((w) => w.id === 'credx-root')?.privateKey ?? '');
  const user = await s.getAddress();
  const cUSD = new ethers.Contract(CONTRACTS.cUSD, CUSD_ABI, s);
  const allowance = await cUSD.allowance(user, CONTRACTS.reputationPerpetual);
  const collateralWei = ethers.parseUnits(collateral.toString(), 18);
  if (allowance < collateralWei) {
    await (await cUSD.approve(CONTRACTS.reputationPerpetual, ethers.parseUnits('1000000', 18))).wait();
  }
  const perp = new ethers.Contract(CONTRACTS.reputationPerpetual, PERP_ABI, s);
  const tx = await perp.openPosition(side === 'LONG' ? 0 : 1, collateralWei, ethers.parseUnits(String(leverage * 10000), 0), { gasLimit: 500000 });
  const receipt = await tx.wait();
  const iface = new ethers.Interface(PERP_ABI);
  let id: string | null = null;
  try {
    const parsed = receipt.logs
      .map((l: any) => (l.address.toLowerCase() === CONTRACTS.reputationPerpetual.toLowerCase() ? iface.parseLog(l) : null))
      .find((p: any) => p && p.name === 'PositionOpened');
    if (parsed) id = parsed.args.id.toString();
  } catch {
    /* best effort */
  }
  return { txHash: String(receipt.hash), block: Number(receipt.blockNumber), id, notional: collateral * leverage };
}

export async function executeClosePerp(id: string, signer?: ethers.Signer): Promise<{ txHash: string; block: number; payout: number }> {
  const s = signer ?? demoWalletSigner(DEMO_WALLET_VAULT.find((w) => w.id === 'credx-root')?.privateKey ?? '');
  const perp = new ethers.Contract(CONTRACTS.reputationPerpetual, PERP_ABI, s);
  const tx = await perp.closePosition(BigInt(id), { gasLimit: 500000 });
  const receipt = await tx.wait();
  let payout = 0;
  try {
    const iface = new ethers.Interface(PERP_ABI);
    const parsed = receipt.logs
      .map((l: any) => (l.address.toLowerCase() === CONTRACTS.reputationPerpetual.toLowerCase() ? iface.parseLog(l) : null))
      .find((p: any) => p && p.name === 'PositionClosed');
    if (parsed) payout = parseFloat(ethers.formatUnits(parsed.args.realizedUsd, 18));
  } catch {
    /* best effort */
  }
  return { txHash: String(receipt.hash), block: Number(receipt.blockNumber), payout };
}

export async function executePerpMargin(id: string, amount: number, deposit: boolean, signer?: ethers.Signer): Promise<{ txHash: string; block: number }> {
  const s = signer ?? demoWalletSigner(DEMO_WALLET_VAULT.find((w) => w.id === 'credx-root')?.privateKey ?? '');
  const user = await s.getAddress();
  if (deposit) {
    const cUSD = new ethers.Contract(CONTRACTS.cUSD, CUSD_ABI, s);
  ...
  const perp = new ethers.Contract(CONTRACTS.reputationPerpetual, PERP_ABI, s);
  const tx = deposit
    ? await perp.addMargin(BigInt(id), ethers.parseUnits(amount.toString(), 18), { gasLimit: 400000 })
    : await perp.removeMargin(BigInt(id), ethers.parseUnits(amount.toString(), 18), { gasLimit: 400000 });
  const receipt = await tx.wait();
  return { txHash: String(receipt.hash), block: Number(receipt.blockNumber) };
}

export async function executeApplyFunding(signer?: ethers.Signer): Promise<{ txHash: string; block: number; ok: boolean }> {
  const s = signer ?? demoWalletSigner(DEMO_WALLET_VAULT.find((w) => w.id === 'credx-root')?.privateKey ?? '');
  const perp = new ethers.Contract(CONTRACTS.reputationPerpetual, PERP_ABI, s);
  const tx = await perp.applyFunding({ gasLimit: 300000 });
  const receipt = await tx.wait();
  return { txHash: String(receipt.hash), block: Number(receipt.blockNumber), ok: true };
}

export interface PerpLedgerEntry {
  type: 'OPEN' | 'CLOSE' | 'LIQUIDATE' | 'MARGIN' | 'FUNDING';
  id: string;
  owner: string;
  side?: 'LONG' | 'SHORT';
  amount: number;
...
export async function fetchPerpLedger(limit = 30): Promise<PerpLedgerEntry[]> {
  try {
    const provider = readProvider();
    const latest = Number(await provider.getBlockNumber());
    const iface = new ethers.Interface(PERP_ABI);
    const out: PerpLedgerEntry[] = [];
    const typeByTopic: Record<string, Exclude<PerpLedgerEntry['type'], 'MARGIN' | 'FUNDING'>> = {
      [PERP_EVENTS.PositionOpened]: 'OPEN',
      [PERP_EVENTS.PositionClosed]: 'CLOSE',
      [PERP_EVENTS.PositionLiquidated]: 'LIQUIDATE',
    };
    for (const [name, topic] of Object.entries(typeByTopic)) {
      let logs: any[] = [];
      for (const win of [1200000, 600000, 300000, 150000, 60000]) {
        try {
          logs = await provider.getLogs({ address: CONTRACTS.reputationPerpetual, topics: [topic], fromBlock: Math.max(1, latest - win), toBlock: 'latest' });
          if (logs.length > 0) break;
        } catch {
          /* narrower window */
        }
      }
      for (const log of logs.reverse().slice(0, limit)) {
        const ev = iface.parseLog(log);
        if (!ev) continue;
        const type = typeByTopic[topic];
        const side: 'LONG' | 'SHORT' | undefined = ev.name === 'PositionLiquidated' ? undefined : Number(ev.args.side) === 0 ? 'LONG' : 'SHORT';
        const amount =
          type === 'OPEN' ? parseFloat(ethers.formatUnits(ev.args.notional, 18))
          : type === 'CLOSE' ? parseFloat(ethers.formatUnits(ev.args.realizedUsd, 18))
          : parseFloat(ethers.formatUnits(ev.args.keeperReward, 18));
        const mark = ev.args.mark !== undefined ? perpPriceToNumber(ev.args.mark) : type === 'OPEN' ? perpPriceToNumber(ev.args.entryMark) : 0;
        const holder = type === 'OPEN' || type === 'CLOSE' ? String(ev.args.owner) : String(ev.args.owner);
        out.push({
          type,
          id: type === 'OPEN' || type === 'CLOSE' ? ev.args.id.toString() : ev.args.id.toString(),
          owner: holder,
          side,
          amount,
          mark,
          ...
          timestamp: Number((await provider.getBlock(log.blockNumber))?.timestamp ?? 0),
        });
      }
    }
    out.sort((a, b) => b.block - a.block);
