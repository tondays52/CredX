...
import { Wifi, ShieldCheck, Database, Cpu, Sparkles, Activity } from 'lucide-react';
import { useProtocol } from '../../context/ProtocolContext';

interface PulseAttestationModalProps {
  isOpen: boolean;
  onClose: () => void;
}

export const PulseAttestationModal: React.FC<PulseAttestationModalProps> = ({ isOpen, onClose }) => {
  const { pulseNetworkQuality, pulseEpoch, pulseBandwidthGB, pulseUptimePoints, pulseNetworkPoints, syncPulseAttestation } = useProtocol();
  ...
  return (
    ...
    >
      <div className="space-y-5 font-sans">
        {/* Banner with Electric Lime accents */}
          ...
          <div className="space-y-1">
            <div className="flex items-center gap-2">
              ...
              <span className="text-[11px] font-mono uppercase tracking-wider text-[#ABF600] font-bold">
                Genesis Epoch {pulseEpoch} Telemetry Ready
              </span>
            </div>
        ...
        <div className="p-4 rounded-2xl bg-black/40 border border-white/[0.08] space-y-3 font-mono text-xs">
          <div className="flex items-center justify-between pb-2 border-b border-white/[0.06] text-white/60">
            ...
            <span className="text-white font-bold">CredX Pulse Bandwidth Layer</span>
          </div>
          <div className="flex items-center justify-between pb-2 border-b border-white/[0.06] text-white/60">
          ...
          <div className="flex items-center justify-between pb-2 border-b border-white/[0.06] text-white/60">
            ...
            <span className="text-cyan-300 font-bold">{pulseBandwidthGB} GB</span>
          </div>
          <div className="flex items-center justify-between pb-2 border-b border-white/[0.06] text-white/60">
            <span>Network Quality Index</span>
            <span className="text-emerald-400 font-bold">{pulseNetworkQuality}% (Zero Proxy / Direct Fiber)</span>
          </div>
          <div className="flex items-center justify-between text-white/60">
            <span>Accumulated Pulse Points</span>
            <span className="text-white font-bold">{(pulseUptimePoints + pulseNetworkPoints).toLocaleString(undefined, { maximumFractionDigits: 2 })} pts</span>
          </div>
        </div>
...
export default PulseAttestationModal;
